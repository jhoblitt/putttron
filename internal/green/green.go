...
// at negative x is above the hole.
type Planar struct {
	slope float64 // tan(slope angle)
	decel float64 // a_d, m/s²
}

func NewPlanar(slopeDeg, decel float64) *Planar {
	return &Planar{slope: math.Tan(slopeDeg * math.Pi / 180), decel: decel}
}
